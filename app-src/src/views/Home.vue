...
<template>
  <AppShell :title="t('appName')">
    ...
    <!-- Impatto ambientale -->
    <h2 class="mb-2 mt-6 px-1 text-sm font-bold text-ink">{{ t('home.impactTitle') }}</h2>
    <section class="grid grid-cols-3 gap-3">
      <StatCard icon="bottle" tone="brand" :value="session.elaborazioni?.bottiglie ?? 0" :label="t('home.bottlesCollected')" />
      <StatCard icon="recycle" tone="accent" :value="session.elaborazioni?.pet ?? 0" :label="t('home.petRecycled')" />
      <StatCard icon="cloud" tone="eco" :value="session.elaborazioni?.co ?? 0" :label="t('home.co2Saved')" />
    </section>

    <!-- Classifica -->
    <h2 class="mb-2 mt-6 px-1 text-sm font-bold text-ink">{{ t('home.ranking') }}</h2>
    ...
    <!-- Ultimi conferimenti -->
    <div class="mb-2 mt-6 flex items-center justify-between px-1">
